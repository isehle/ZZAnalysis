        ...
        for reg in tqdm(self.regions, desc = "Regions", position = 0):
            ...
            for prop in tqdm(self.properties, desc = "Properties", position = 1, leave = False):
                ...
                for fs in tqdm(self.final_states, desc = "Final States", position = 2, leave = False):
                    if not self.data:
                        hist = self.fs_filt(df, reg, fs).Histo1D((good_branch, good_branch, int(hist_info["nbinsx"]), float(hist_info["xlow"]), float(hist_info["xhigh"])), good_branch, weight_col) 

                    else:
                        hist = self.fs_filt(df, reg, fs).Histo1D((good_branch, good_branch, int(hist_info["nbinsx"]), float(hist_info["xlow"]), float(hist_info["xhigh"])), good_branch)
